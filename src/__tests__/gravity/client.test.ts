...
describe('GravityClient', () => {
  ...
  beforeEach(() => {
    ...
    mockGrpcClient = {
      GetGravityTasks: jest.fn((params, callback) => callback(null, { gravityTaskStates: [] })),
      GetCrawler: jest.fn((params, callback) => callback(null, { crawler: {} })),
      CreateGravityTask: jest.fn((params, callback) => callback(null, { gravityTaskId: 'test-id' })),
      BuildDataset: jest.fn((params, callback) => callback(null, { datasetId: 'test-id', dataset: {} })),
      GetDataset: jest.fn((params, callback) => callback(null, { dataset: {} })),
      CancelGravityTask: jest.fn((params, callback) => callback(null, { message: 'Cancelled' })),
      CancelDataset: jest.fn((params, callback) => callback(null, { message: 'Cancelled' }))
    };

    // Replace the createGrpcClient method to return our mock
    jest.spyOn(client as any, 'createGrpcClient').mockReturnValue(mockGrpcClient);
  });
  ...
  describe('getGravityTasks', () => {
    ...
    it('should handle errors', async () => {
      mockGrpcClient.GetGravityTasks.mockImplementationOnce((params: any, callback: (error: Error | null, response: any) => void) => 
  ...
  describe('cancelGravityTask', () => {
    it('should call the CancelGravityTask method on the gRPC client', async () => {
      const result = await client.cancelGravityTask({ gravityTaskId: 'test-id' });
